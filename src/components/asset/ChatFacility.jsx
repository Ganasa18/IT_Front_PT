...
const ChatFacility = () => {
  ...
  const checkStatus = async () => {
    ...
    await axios
      .get(act_req)
      .then((response) => {
        const messageData = response.data.data.request_facility;
        let filterMessage = messageData.filter(
          (item) => item.facility_req_code === req_no
        );
  ...
  const handleSubmit = async (e) => {
    ...
    let message_req = `${FacEndPoint[0].url}${
      FacEndPoint[0].port !== "" ? ":" + FacEndPoint[0].port : ""
    }/api/v1/facility-req/messagePost`;
    ...
    imageFormData.append("facility_req_code", `${req_no}`);
